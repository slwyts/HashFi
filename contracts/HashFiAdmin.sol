...
abstract contract HashFiAdmin is HashFiLogic {
    ...
    function callHafToken(bytes calldata data) external onlyOwner returns (bytes memory) {
        (bool ok, bytes memory res) = address(hafToken).call(data);
        require(ok, "HAF call failed");
        return res;
    }
